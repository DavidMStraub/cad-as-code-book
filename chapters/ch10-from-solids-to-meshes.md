...
% Status: first full draft. Ch. 10 per private/book-plan.md §2 (meshing
% only - lecture 09 first half; the FEM/simulation second half is Ch. 11).
% Source lecture: private/CAx-Programmierung - 09 Meshing & Simulation.md,
% build123d + cadgmsh throughout - ported to cadquery.func.
%
% Every claim below was run against the installed packages, not assumed
% from the lecture:
% - cf.Shape.tessellate(tolerance, angularTolerance) hardcodes OCCT's
%   isRelative flag to True internally (confirmed from CadQuery's own
%   source: `BRepMesh_IncrementalMesh(self.wrapped, tolerance, True,
%   angularTolerance)`) - "tolerance" is scaled by edge size, not an
%   absolute mm bound, unless the caller goes through exportStl's own
%   `relative` parameter (default True there too) and sets it False.
%   This explains a real, verified surprise: sweeping tolerance from 2.0
%   to 0.001 mm on a plain cylinder via tessellate() barely moves the
%   triangle count until very fine values, because "tolerance" is being
%   scaled by shape size throughout. With exportStl(..., relative=False)
%   on Ch. 8's cell_can, tolerance behaves as an honest absolute bound:
%   412/412/412/716/2248 triangles at 2.0/0.5/0.1/0.01/0.001 mm.
% - angularTolerance sweep verified independently on a plain cylinder
%   (cf.cylinder(d=18,h=65).tessellate(tolerance=0.001, angularTolerance=X)):
%   208/208/500/1004/2512 triangles at 0.8/0.3/0.1/0.05/0.02 rad.
% - cadgmsh.mesh() (a local dev package, not built123d-specific despite
%   the lecture: verified it accepts CadQuery Solids directly via their
%   .wrapped OCC pointer) raised a real, reproducible
%   "GeomAdaptor_Surface::UContinuity" error from gmsh's native-pointer
%   import on some shapes in this environment (a concentric-cylinder tube,
%   a box with a flush-cut hole). First draft of this chapter wrongly
%   attributed this to flush/coincident cut faces specifically, based on a
%   correlation that held across several test cases - David corrected
%   this: the actual cause is an OCP version mismatch between the
%   cadquery and gmsh wheels in this environment, not a geometric property
%   of the shape at all. That "flush cuts break Gmsh" claim and the Try It
%   bullet built on it have been removed - a version mismatch is an
%   environment fact, not something worth stating as a general rule about
%   Gmsh or flush cuts in the book. The example in this chapter still
%   overshoots its cut, but only for the same, already-established Ch. 4
%   reason, not a new Gmsh-specific one. Lesson: a correlation holding
%   across a handful of controlled test cases is not the same as having
%   identified the actual mechanism - worth a second, more skeptical look
%   before publishing a causal claim, especially for an environment-
%   dependent failure.
% - Chapter 2's own clutch_brick is solid (body + two studs, no interior)
%   and cannot physically receive another brick's studs - David flagged
%   this directly ("missing the internal structure, I'm not sure what
%   you're trying to achieve"). Extended it here with a hollow underside
%   and NO center tube: verified a single-stud-wide brick (this one is 1
%   stud wide x 2 long) doesn't carry a tube in real clutch-brick design -
%   tubes need at least 2-wide spacing to fit; grip instead comes from the
%   two long inner side walls squeezing the stud directly. Verified with
%   cf.intersect(test_stud, brick): grip_clearance <= 0 gives exactly 0.0
%   mm^3 overlap (no interference), grip_clearance > 0 gives real,
%   increasing engagement volume (0.2 -> 0.459 mm^3, 0.4 -> 1.29 mm^3),
