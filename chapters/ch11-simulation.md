...
% Status: first full draft. Ch. 11 per private/book-plan.md §2 (lecture 09,
% second half - the FEM/simulation content the meshing half of that lecture
% led into). Two worked examples per David's explicit request: a mechanical
% one (a tension rod, book-plan's own long-committed choice, validated
% against the analytic solution) and a thermal one (three of Chapter 3/4's
% own cells, each generating heat internally and cooled by a fixed-
% temperature plate plus convection - not in the source lecture at all,
% went through two rejected drafts before landing here, see below).
%
% Every claim and number below was run against the installed packages
% (CadQuery 2.8.0, cadgmsh, scikit-fem 11.0.0), not assumed:
% - Tension rod (steel, E=210e3 MPa, nu=0.3, r=10mm, l=40mm, F=1000N axial):
%   named face groups via cadgmsh's own `physical={...}` work directly on a
%   plain cylinder, no workaround needed. Mean von Mises stress across the
%   whole mesh (4143 elements, lc=2.5) came out 3.155 MPa against the
%   analytic F/A = 3.183 MPa - under 1% - with real, visible local spread
%   (1.74 to 6.21 MPa) concentrated at the loaded face, exactly the
%   plausibility-check-not-blind-trust framing book-plan asks to keep.
%   Displacement: mid-span nodal values (0.262-0.321 um) bracket the
%   analytic value at that height (0.303 um); near the loaded face the
%   range is wider (0.563-0.687 um against an analytic 0.606 um) - a real,
%   honest St. Venant's-principle effect from applying the load as a
%   distributed nodal force at discrete points rather than a smooth
%   pressure, not a bug.
% - Thermal example went through real revision. First draft modeled the
%   tray alone with its pocket walls fixed at a hot temperature, standing
%   in for cell contact - David: "I don't want tray heating, I want cell
%   heating!", correct, that model put the boundary condition on the wrong
%   body's surface. Second draft fuses cells into the tray as one solid so
%   the mesh is automatically conformal - David: "Bullshit to fuse cells
%   and tray. They have completely different thermal properties," also
%   correct, a single-material mesh cannot represent two materials with
%   different conductivities regardless of how convenient the meshing is.
%   Final version: a compound of three separate, unconnected cells (no
%   tray at all), each with its own volumetric heat generation, a fixed-
%   temperature "cooling plate" boundary condition on its own bottom cap,
%   and convective cooling on its sides/top. Investigated (per David's own
%   question) whether it's defensible to ignore a real busbar/thermal
%   connection between cells: verified computationally that for identical,
%   equally-loaded cells a connecting busbar carries essentially zero heat
%   (88.48 degC connected vs. 88.4 degC disconnected, by symmetry - no
%   temperature difference, no driving force), but that a busbar measurably
%   helps once cells are loaded unevenly (151.85 degC isolated vs. 147.65
%   degC connected, one cell at 2x the other's generation). David's
%   explicit call: leave the busbar/asymmetric case out of the chapter
%   entirely, do not mention it even as a caveat - kept here only as a
%   private record of why the omission was actually checked, not assumed.
% - Verified numbers, final version: k=1.0e-3 W/(mm K) (illustrative
%   effective cell conductivity), q=3.0e-5 W/mm^3 (illustrative uniform
%   generation, roughly 0.5 W per cell), h_conv=1.5e-5 W/(mm^2 K) (roughly
%   15 W/(m^2 K), natural still-air convection), T_ambient=T_cooling_plate
%   =25 degC. Solved field: 25.0 to 33.13 degC, mean 30.95 degC - a small,
%   physically sane rise, cross-checked against a 1D insulated-rod analytic
%   approximation (without convection: 88.41 degC computed vs. 88.375 degC
%   analytic, well under 1% - convection is what pulls the real, final
%   model's numbers down from there).
% - Cell geometry later gained a terminal boss (r=2.5, h=1.0, matching
%   Ch. 3's own 18650 knob dimensions) per David's request for a more
%   recognizable cell silhouette in the figure - re-verified as part of
%   the actual meshed/solved geometry, not just the render: min/max/mean
%   moved from 25.0/33.134/30.954 to 25.0/33.117/30.987, an under-0.1%
%   shift, so the chapter's own rounded numbers (25, 33.1) did not need
%   to change. The true hottest point (on-axis, ~70% up the height) also
%   caught a real mistake in an earlier draft of this section: it had
%   claimed the top cap was the hottest point and that it was visible on
%   the outer surface, neither true - the surface only reaches ~33.09
%   degC at its warmest, the true max is interior. Fixed by clipping the
%   render to show the interior and correcting the prose.
% - cadgmsh.mesh() accepts a list of shapes directly (verified: a compound
%   of 3 non-touching cells meshes in one call, each keeping its own
%   physical-group identity) - no tray-meshing workaround needed anymore;
%   David's own upstream fix (BREP roundtrip + BREP-consistent ShapeIndex,
%   replacing native-pointer import) resolved the crash this chapter's
%   Ch. 10-era draft hit on blind pockets. One separate, real limitation
%   found and NOT worked around, because this chapter's final model does
%   not need it: `imprint=True` (conformal meshing of touching bodies)
