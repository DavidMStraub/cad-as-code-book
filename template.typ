// ─────────────────────────────────────────────────────────────────────────────
// template.typ — Book layout and reusable components
// ─────────────────────────────────────────────────────────────────────────────

// ── Colours ──────────────────────────────────────────────────────────────────
#let accent       = rgb("#4a6fa5") // muted steel blue
#let bg-note      = rgb("#eff6ff") // blue-50
#let bg-warn      = rgb("#fff7ed") // orange-50
#let bg-tip       = rgb("#f0fdf4") // green-50
#let border-note  = rgb("#3b82f6")
#let border-warn  = rgb("#f97316")
#let border-tip   = rgb("#22c55e")
#let code-bg      = rgb("#f5f5f5") // light grey
#let code-fg      = rgb("#1e1e2e") // near-black

// ── Fonts ─────────────────────────────────────────────────────────────────────
#let body-font  = ("Linux Libertine O",)
#let mono-font  = ("Fira Code", "IBM Plex Mono")
#let sans-font  = ("Source Sans 3", "IBM Plex Sans", "Linux Biolinum O")

// ── Page geometry ─────────────────────────────────────────────────────────────
#let page-setup = (
  paper: "us-letter",
  margin: (top: 2.5cm, bottom: 2.5cm, inside: 3cm, outside: 2cm),
  binding: left,
)

// ─────────────────────────────────────────────────────────────────────────────
// Callout helper
// ─────────────────────────────────────────────────────────────────────────────
#let callout(icon: "ℹ", bg: bg-note, border: border-note, title: none, body) = block(
  width: 100%,
  fill: bg,
  stroke: (left: 3pt + border),
  inset: (left: 12pt, right: 10pt, top: 8pt, bottom: 8pt),
  radius: (right: 4pt),
)[
  #if title != none [
    #text(weight: "bold", fill: border, size: 0.9em)[#icon #title]
    #v(4pt)
  ]
  #body
]

// ── Public callout shorthands ─────────────────────────────────────────────────
#let note(title: "Note", body)    = callout(icon: "ℹ", bg: bg-note, border: border-note, title: title, body)
#let warning(title: "Warning", body) = callout(icon: "⚠", bg: bg-warn, border: border-warn, title: title, body)
#let tip(title: "Tip", body)      = callout(icon: "✓", bg: bg-tip,  border: border-tip,  title: title, body)

// ─────────────────────────────────────────────────────────────────────────────
// Code example block
// ─────────────────────────────────────────────────────────────────────────────
#let code-example(caption: none, lang: "python", body) = figure(
  caption: caption,
  block(
    width: 100%,
    fill: code-bg,
    stroke: 0.5pt + rgb("#d0d0d8"),
    radius: 6pt,
    inset: 14pt,
    text(font: mono-font, size: 0.82em, fill: code-fg, raw(body, lang: lang)),
  ),
  kind: "listing",
  supplement: [Listing],
)


// ─────────────────────────────────────────────────────────────────────────────
// Book show rule — apply global formatting
// ─────────────────────────────────────────────────────────────────────────────
#let book(
  title: "",
  subtitle: "",
  author: "",
  edition: "",
  year: "",
  publisher: "",
  isbn: "",
  doi: "",
  urn: "",
  doc,
) = {
  // ── Document metadata ──────────────────────────────────────────────────────
  set document(title: title + ": " + subtitle, author: author)

  // ── Page layout ───────────────────────────────────────────────────────────
  set page(
    ..page-setup,
    header: context {
      let page-num = counter(page).get().first()
      if page-num > 2 [
        #set text(size: 0.78em, fill: luma(120))
        #if calc.odd(page-num) [
          #h(1fr) #smallcaps[#title] #h(6pt) #page-num
        ] else [
          #page-num #h(6pt) #smallcaps[#author] #h(1fr)
        ]
        #v(-4pt)
        #line(length: 100%, stroke: 0.3pt + luma(180))
      ]
    },
    footer: [],
  )

  // ── Typography ────────────────────────────────────────────────────────────
  set text(font: body-font, size: 11pt, lang: "en")
  set par(justify: true, leading: 0.75em, first-line-indent: 0em)
  set heading(numbering: "1.1.1.")

  // ── Heading hierarchy ────────────────────────────────────────────────────
  show heading: it => { set par(first-line-indent: 0em); it }

  // Level 1 — Chapter
  show heading.where(level: 1): it => {
    pagebreak(weak: true)
    v(2cm)
    if it.numbering != none [
      #text(font: sans-font, size: 1em, fill: accent, weight: "bold")[
        CHAPTER #counter(heading).display("1")
      ]
      #v(4pt)
    ]
    text(font: sans-font, size: 2.2em, weight: "bold")[#it.body]
    v(0.4cm)
    line(length: 100%, stroke: 0.5pt + accent)
    v(0.6cm)
  }
  // Level 2 — Section
  show heading.where(level: 2): it => {
    set par(first-line-indent: 0em)
    v(1.4em)
    text(font: sans-font, size: 1.5em, weight: "semibold", fill: accent)[#it.body]
    v(0.3em)
  }
  // Level 3 — Subsection
  show heading.where(level: 3): it => {
    set par(first-line-indent: 0em)
    v(1em)
    text(font: sans-font, size: 1.2em, weight: "regular")[#it.body]
    v(0.2em)
  }
  // Level 4 — Sub-subsection
  show heading.where(level: 4): it => {
    set par(first-line-indent: 0em)
    v(0.7em)
    text(font: sans-font, size: 1em, weight: "regular", style: "italic")[#it.body]
    v(0.15em)
  }

  // ── Code blocks ───────────────────────────────────────────────────────────
  show raw.where(block: true): it => block(
    width: 100%,
    fill: code-bg,
    stroke: 0.5pt + rgb("#d0d0d8"),
    radius: 6pt,
    inset: 14pt,
    text(font: mono-font, size: 0.82em, fill: code-fg, it),
  )
  show raw.where(block: false): it => highlight(
    fill: rgb("#ebebf0"),
    top-edge: 0.7em,
    bottom-edge: -0.2em,
    radius: 2pt,
    text(font: mono-font, size: 0.87em, fill: code-fg, it),
  )

  // ── Figure captions ───────────────────────────────────────────────────────
  show figure.caption: it => text(size: 0.87em, style: "italic")[#it]

  // ── Title page ────────────────────────────────────────────────────────────
  page(
    margin: (top: 4cm, bottom: 3cm, left: 3.5cm, right: 3.5cm),
    header: [],
    footer: [],
  )[
    #v(2fr)
    #align(center)[
      #text(font: sans-font, size: 3em, weight: "black", fill: accent)[#title]
      #v(0.4cm)
      #text(font: sans-font, size: 1.5em, fill: luma(60))[#subtitle]
      #v(1.2cm)
      #line(length: 40%, stroke: 1.5pt + accent)
      #v(1.2cm)
      #text(font: sans-font, size: 1.2em)[#author]
      #v(0.3cm)
      #text(font: sans-font, size: 0.9em, fill: luma(100))[#edition]
    ]
    #v(3fr)
    #align(center)[
      #text(size: 0.82em, fill: luma(120))[
        #publisher #h(1em) · #h(1em) #year \
        #if isbn != "" [ISBN #isbn \ ]
        #if doi  != "" [DOI #doi \ ]
        #if urn  != "" [URN #urn]
      ]
    ]
  ]

  // ── Table of contents ─────────────────────────────────────────────────────
  page(header: [], footer: [])[
    #text(font: sans-font, size: 2em, weight: "bold")[Contents]
    #v(1.5em)
    #show outline.entry.where(level: 1): it => {
      v(1em, weak: true)
      strong(it)
    }
    #outline(
      title: none,
      indent: auto,
      depth: 2,
    )
  ]

  // ── Body ──────────────────────────────────────────────────────────────────
  counter(page).update(1)
  doc
}
